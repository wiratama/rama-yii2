	...
	public function actionPromo()
	{
		...
		$query = Product::find()->joinWith([
					'productCategories' => function ($query) {
						$query->andWhere(['=', 'id_category', Yii::$app->user->identity->id_category]);
					},
				])->with('productCategories.idCategory')->where('status=1 and (start_date < NOW() - INTERVAL 1 DAY) and (end_date > NOW() - INTERVAL 1 DAY)');
